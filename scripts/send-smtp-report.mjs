...
/**
 * Reads an environment variable and throws a descriptive error if it is missing.
 *
 * This script is intended to run in CI where missing configuration should fail fast.
 */
...
/**
 * Returns the URL for the current GitHub Actions run, if the standard environment variables exist.
 *
 * @returns {string|null}
 */
 ...
 *
 * This is used to render an Outlook-compatible HTML email body.
 */
...
/**
 * Reads a UTF-8 text file and returns `null` if it cannot be read.
 *
 * This is used for optional artifacts like `playwright-summary.txt`.
 */
...
/**
 * Parses Playwright list reporter output and extracts summary stats and unique failure titles.
 *
 * This logic intentionally strips ANSI and matches common list reporter patterns.
 */
...
/**
 * Normalizes test titles so they can be compared across slightly different renderings.
 *
 * The goal is "loose equality" that survives path separator differences and whitespace noise.
 */
...
/**
 * Strips quotes around values extracted from summary lines.
 *
 * Playwright/expect output can include either single or double quotes.
 */
...
/**
 * Heuristic: determines whether a failure appears to relate to CSV output.
 *
 * This impacts how we interpret "received/expected" values when the summary lacks explicit "Shown on CSV:" lines.
 */
...
/**
 * Extracts failure details from Playwright list reporter output.
 *
 * The goal is to produce user-friendly email cards with:
 * - what went wrong
 * - optional reason
 * - shown values (site/CSV) when present or inferable
 * - expected values (exact or one-of)
 */
...
/**
 * Formats the "Shown on site" line for text/HTML output.
 *
 * For some failure types (non-CSV assertions), the CSV field is intentionally not applicable.
 */
...
/**
 * Formats the "Shown on CSV" line for text/HTML output.
 */
...
/** Returns the value portion of the formatted "Shown on site" line for the email cards table. */
...
/** Returns the value portion of the formatted "Shown on CSV" line for the email cards table. */
...
/**
 * Attempts to compute the same normalized key that Playwright uses for a given test result directory.
 *
 * This reads `error-context.md` generated by Playwright to correlate screenshots to test names.
 */
...
/**
 * Splits a full Playwright test id into a "headline" (leaf title) and "suite" breadcrumb.
 *
 * Used to render email cards that are readable at a glance.
 */
...
/**
 * Converts Playwright/GitHub status into a presentable word for the email subject.
 */
...
/**
 * Returns a display string for the run timestamp (UTC) unless overridden by RUN_DISPLAY_TIME.
 *
 * This allows the workflow to inject a human-friendly locale time if desired.
 */
...
/**
 * Builds the plaintext "details" section for failures/flaky failures.
 *
 * The HTML version includes richer formatting and optional inline screenshots.
 */
